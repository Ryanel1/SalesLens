...
async function matchingImage(item: ImageRequestItem): Promise<ProductImageMatch | null> {
  ...
  const productUrls = lookup.productUrl
    ? [lookup.productUrl]
    : await productDetailUrlsForArt(lookup.searchArtCode);

  for (const productUrl of productUrls.slice(0, 50)) {
    const detailHtml = await fetchText(productUrl).catch(() => "");
    if (!detailHtml) continue;

    if (!lookup.productUrl && !detailMatches(detailHtml, item, lookup.searchArtCode)) {
      continue;
    }
...
async function productDetailUrlsForArt(artCode: string) {
  const searchUrl = `${REBEL_RAGS_BASE_URL}/all-products/browse/keyword/${encodeURIComponent(artCode)}`;
...
function detailMatches(html: string, item: ImageRequestItem, lookupArtCode: string) {
  const compactHtml = normalized(html);
  const style = normalized(item.style);
  const artCode = normalized(lookupArtCode);

  if (compactHtml.includes(style + artCode) || compactHtml.includes(artCode + style)) {
    return true;
  }

  return compactHtml.includes(style) && compactHtml.includes(artCode);
}
